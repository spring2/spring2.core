...
function $unlink(object){
	...
	switch ($type(object)){
		...
		break;
		case 'hash':
			//JeffM - Changed from mootools released code (6/12/2008)
			//Changed to call $unlink for all keys and values and not just return clean object.
			//OLD CODE -- unlinked = $unlink(object.getClean());
			unlinked = new Hash;
			Hash.each(object, function(value, key){
			    unlinked.set($unlink(key), $unlink(value));
			});
		break;
		case 'array':
			unlinked = [];
