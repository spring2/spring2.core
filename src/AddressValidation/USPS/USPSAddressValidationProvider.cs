...
namespace Spring2.Core.AddressValidation.USPS {
    public class USPSAddressValidationProvider: IAddressValidationProvider {

	/*private string USPSAccessKey {
	    get { return ConfigurationProvider.Instance.Settings["USPS.AccessKey"]; }
	}

	private string USPSUserId {
	    get { return ConfigurationProvider.Instance.Settings["USPS.UserId"]; }
	}

	private string USPSPassword {
	    get { return ConfigurationProvider.Instance.Settings["USPS.Password"]; }
	}

	private string USPSServer {
	    get { return ConfigurationProvider.Instance.Settings["USPS.Server"]; }
	}*/
	...
	public AddressValidationResult Validate(StringType street1, StringType street2, StringType street3, StringType city, StringType state, StringType postalCode, StringType countryCode) {
	    ...
	    result.ResponseType = IsMilitary(city,state,postalCode) ? ResponseTypeEnum.VALID : ResponseTypeEnum.INVALID;
	    return result;
	}

	// This is more of a "best guess" than a verification that the address is military
	private bool IsMilitary(String city, String region, String postalCode) {
	    ...
	    bool isCityMilitary = (city == "APO" || city == "FPO");
	    bool isRegionMilitary = (region == "AA" || region == "AE" || region == "AP");
	    bool isPostalCodeMilitary = false;

	    if(isCityMilitary && isRegionMilitary) {
		switch (region) { 
		    case "AA":
			isPostalCodeMilitary = postalCode.StartsWith("34") && postalCode.Length >= 5;
			break;
		    case "AE":
			isPostalCodeMilitary = postalCode.StartsWith("09") && postalCode.Length >= 5;
			break;
		    case "AP":
			isPostalCodeMilitary = postalCode.StartsWith("96") && postalCode.Length >= 5;
			break;
		}
	    }

	    isMilitary = isCityMilitary && isRegionMilitary && isPostalCodeMilitary;
	    return isMilitary;
	}
	...
	public AddressValidationResult Validate(StringType street1, StringType street2, StringType street3, StringType city, StringType state, StringType postalCode, StringType countryCode) {
	    ...
	    HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create(USPSServer);
	...
	private string AccessRequestXml {
		...
		return string.Format("<?xml version=\"1.0\"?>" +
		    ...
		    "<UserId>{1}</UserId>" +
		    "<Password>{2}</Password>" +
		    "</AccessRequest>", USPSAccessKey, USPSUserId, USPSPassword);
	    }
	}
